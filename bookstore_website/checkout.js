document.addEventListener("DOMContentLoaded", () => {
  ...
  function render() {
    ...
    document.querySelectorAll("[data-address]").forEach(
      (el) =>
        (el.onclick = () => {
          selectedAddress = el.dataset.address;
          render();
        }),
    );
  ...
  function renderSummary() {
    ...
    summary.innerHTML = `
  <h3 style="margin-top:0;color:var(--brown)">
    สรุปยอดชำระ
  </h3>
  ...
  ${items
    ...
    <div class="summary-row">
      <span>
        ${BookApp.escapeHtml(i.product.title)} × ${i.qty}
      </span>
      <strong>
        ${BookApp.formatTHB(i.product.price * i.qty)}
      </strong>
    </div>
  `,
    )
  ...
  <p class="helper">
    ที่อยู่: ${BookApp.escapeHtml(addressText)}
  </p>
...
`;
    ...
    document.getElementById("payBtn").onclick = () => {
      if (!address) {
        BookApp.toast("กรุณาเพิ่มที่อยู่จัดส่ง");
        return;
      }

      if (ship.id === "unknown") {
        BookApp.toast("กรุณาเลือกวิธีจัดส่ง");
        return;
      }
